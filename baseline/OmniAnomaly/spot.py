...
class SPOT:
    ...
    def _grimshaw(self, epsilon=1e-8, n_points=10):
        ...
        def w(Y, t):
            s = 1 + t * Y
            s = np.where(np.abs(s) < 1e-10, 1e-10, s)  # 防止s接近0
            us = u(s)
            vs = v(s)
            result = us * vs - 1
            return np.nan_to_num(result)  # 转换NaN为数值
        ...
        def jac_w(Y, t):
            ...
            safe_t = np.maximum(t, 1e-10) # 确保分母不为零 
            safe_s = np.where(np.abs(s) < 1e-10, 1e-10, s) # 处理s中的零值 
            jac_us = (1 / safe_t) * (1 - vs) 
            jac_vs = (1 / safe_t) * (-vs + np.mean(1 / np.square(safe_s)))
...
class biSPOT:
    ...
    def _grimshaw(self, side, epsilon=1e-8, n_points=10):
        ...
        def jac_w(Y, t):
            ...
            us = u(s)
            vs = v(s)
            safe_t = np.maximum(np.abs(t), 1e-10)  # 确保分母不为零且保持符号
            jac_us = (1 / safe_t) * (1 - vs)
            jac_vs = (1 / safe_t) * (-vs + np.mean(1 / np.square(s)))
            result = us * jac_vs + vs * jac_us
            return np.nan_to_num(result)  # 转换NaN为数值
